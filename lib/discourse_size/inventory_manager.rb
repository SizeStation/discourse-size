...
module ::DiscourseSize
  class InventoryManager
    ...
    def self.with_character_locks(character_ids, &block)
      ids = Array(character_ids).compact.uniq.sort
      return yield if ids.empty?

      acquire_locks = ->(remaining_ids) do
        if remaining_ids.empty?
          yield
        else
          id = remaining_ids.first
          DistributedMutex.synchronize("discourse_size_character_#{id}") do
            acquire_locks.call(remaining_ids.drop(1))
          end
        end
      end
