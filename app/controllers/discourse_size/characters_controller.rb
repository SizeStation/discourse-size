  ...
  class CharactersController < ::ApplicationController
    ...
    def index
      if params[:q].present?
        ...
        return(
          render json: {
                   characters:
                     serialize_data(characters.limit(20), DiscourseSizeCharacterSerializer),
                 }
        )
      end

      user_id =
        params[:user_id] || (params[:username] && User.find_by(username: params[:username])&.id) ||
          current_user&.id
      ...
      characters =
        DiscourseSizeCharacter.where(user_id: user_id).order(
          is_main: :desc,
          position: :asc,
          created_at: :asc,
        )
    ...
    def reorder
      ...
      if params[:folder_id] && params[:character_ids]
        ...
        DiscourseSizeCharacter.where(id: params[:character_ids], user_id: user_id).update_all(
    ...
    def reorder_top_level
      ...
      mapping.each_with_index do |item, index|
        if item[:type] == "character"
          ...
          DiscourseSizeFolder.where(id: item[:id], user_id: user_id).update_all(position: index)
        end
      end
    ...
    def set_size
      ...
      unless character.normal? || current_user.admin?
        return(
          render json: failed_json.merge(error: "Only Normal characters can set size directly"),
                 status: :forbidden
        )
      end
    ...
    def trigger
      ...
      unless character.user_id == current_user.id || current_user.admin?
        rp_ids =
          character.discourse_size_roleplay_members.where(status: "accepted").pluck(:roleplay_id)
        user_char_ids = DiscourseSizeCharacter.where(user_id: current_user.id).pluck(:id)
        shared_rp =
          DiscourseSizeRoleplayMember.where(
      ...
      result =
        DiscourseSize::TriggerExecutor.execute(character, params[:trigger_name], current_user)
      ...
      if action.parent_action_id.present? && !current_user.admin?
        return(
          render json: {
                   failed: true,
                   message:
                     "This activity entry is linked to another character's interaction and cannot be deleted individually.",
                 },
                 status: :unprocessable_content
        )
      end
    ...
    def unblock_user
      ...
      character.blocked_user_ids =
        character.blocked_user_ids.map(&:to_i).reject { |id| id == target_id }
      character.save!
    ...
    def character_params
      params.permit(
        ...
        blocked_item_keys: [],
        blocked_user_ids: [],
        discourse_size_character_properties_attributes: %i[id name property_type value _destroy],
        discourse_size_character_triggers_attributes: %i[id name js_code _destroy],
      )
    end

    def folder_serializer(f)
      { id: f.id, name: f.name, position: f.position, user_id: f.user_id, hex_color: f.hex_color }
    end
  end
end
