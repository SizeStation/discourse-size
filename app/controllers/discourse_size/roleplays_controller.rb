...
module DiscourseSize
  class RoleplaysController < ::ApplicationController
    ...
    def index
      roleplay_ids =
        DiscourseSizeRoleplayMember
          .joins(:character)
          .where(discourse_size_characters: { user_id: current_user.id })
          .select(:roleplay_id)

      roleplays =
        DiscourseSizeRoleplay.where(creator_id: current_user.id).or(
          DiscourseSizeRoleplay.where(id: roleplay_ids),
        )
    ...
    def remove_member
      ...
      unless roleplay.creator_id == current_user.id ||
               member.character.user_id == current_user.id || current_user.admin?
    ...
    def create
      ...
      if roleplay.save
        ...
        render json: failed_json.merge(errors: roleplay.errors.full_messages),
    ...
    def update
      roleplay = find_roleplay
      unless roleplay.creator_id == current_user.id || current_user.admin?
        raise Discourse::InvalidAccess
      end
    ...
    def join
      ...
      unless roleplay.discourse_size_roleplay_members.exists?(character_id: character.id)
        roleplay.discourse_size_roleplay_members.create!(
    ...
    def invite
      ...
      if MutedUser.where(user_id: current_user.id, muted_user_id: target_user.id).exists? ||
        ...
        return(
          render json:
                   failed_json.merge(
                     errors: [I18n.t("discourse_size.roleplays.errors.user_blocked_by_you")],
                   ),
                 status: :forbidden
        )
      end
      ...
      if MutedUser.where(user_id: target_user.id, muted_user_id: current_user.id).exists? ||
        ...
        return(
          render json:
                   failed_json.merge(
                     errors: [I18n.t("discourse_size.roleplays.errors.user_blocking")],
                   ),
                 status: :forbidden
        )
      end
    ...
    def accept_invite
      member =
        DiscourseSizeRoleplayMember.find_by(
          roleplay_id: find_roleplay.id,
    ...
    def update_member_overrides
      roleplay = find_roleplay
      member = roleplay.discourse_size_roleplay_members.find(params[:member_id])
      unless member.character.user_id == current_user.id || current_user.admin?
        raise Discourse::InvalidAccess
      end

      overrides =
        member.override_data.merge(JSON.parse(params.permit!.to_json).slice(*OVERRIDE_KEYS))
    ...
    def find_roleplay
      DiscourseSizeRoleplay.find_by(uuid: params[:id]) ||
        DiscourseSizeRoleplay.find_by(id: params[:id]) || (raise Discourse::NotFound)
    end
