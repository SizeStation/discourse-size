...
module DiscourseSize
  class ShopController < ::ApplicationController
    ...
    before_action :ensure_logged_in, only: %i[purchase claim_reward save_settings]
    ...
    def purchase
      ...
      if !item || !item.in_stock?
        return(
          render json: {
                   failed: true,
                   message: "Item is out of stock or disabled.",
                 },
      ...
      result =
        ::DiscourseSize::InventoryManager.purchase(current_user, item_key, guardian: guardian)
      ...
      if result[:success]
        ...
        render json: {
                 success: true,
                 inventory_item:
                   serialize_data(result[:inventory_item], DiscourseSizeInventorySerializer),
      ...
      DistributedMutex.synchronize("discourse_size_daily_reward_#{current_user.id}") do
        ...
        if last_reward_date == today
          return(
            render json: {
                     failed: true,
                     message: "Reward already collected today.",
                   },
                   status: :unprocessable_content
          )
        end
    ...
    def save_settings
      ...
      settings.update!(measurement_system: params[:measurement_system])
      render json: success_json
    end
