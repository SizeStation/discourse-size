...
module ::DiscourseSize
  class SizeController < ::ApplicationController
    ...
    def spend_points
      ...
      target_user = target_username.present? ? User.find_by_username(target_username) : current_user
      ...
      change_multiplier = action == 'grow' ? (1.0 + (total_percent_change / 100.0)) : (1.0 - (total_percent_change / 100.0))
    ...
    def compare
      ...
      stats = users.map do |u|
        stat = u.user_size_stat
        {
          ...
          upload_url: stat.character_upload_id ? Upload.find_by(id: stat.character_upload_id)&.url : u.avatar_template
        }
      end
