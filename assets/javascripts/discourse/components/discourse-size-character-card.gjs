...
import DiscourseSizeCharacterActions from "./discourse-size-character-actions";
...
export default class DiscourseSizeCharacterCard extends Component {
    ...
    return Object.values(grouped).map((group) => {
      ...
      group._queueText = (group.queued || [])
        .map((q) => {
          ...
          return (
            parent?.item_name || parent?.item_key || q.item_name || q.item_key
          );
        })
        .join(", ");
  ...
  get formattedQueuedActions() {
    ...
    return this.queuedActions
      .map((a) => {
        return a.item_name || i18n("discourse_size.unknown");
      })
      .join(", ");
  }
  ...
  <template>
    <div
      class="discourse-size-character-card {{if @character.is_main 'is-main'}}"
    >
        ...
        <div class="character-info">
          ...
          <div class="character-header-meta">
            ...
            {{#unless this.siteSettings.discourse_size_disable_properties}}
              {{#each this.effectiveProperties as |prop|}}
                <span
                  class="header-meta-card custom-prop {{prop.property_type}}"
                >
          ...
          {{#if this.isAnimating}}
            <div class="character-size-row">
              <div class="growth-stats">
                ...
                {{#unless this.siteSettings.discourse_size_disable_properties}}
                  {{#each this.allPropertyActions as |prop|}}
                    <div class="property-progress-bar-wrapper">
                      {{#if prop.active}}
                        <div class="growth-status-label">
                          <span
                            class={{if
                              (gt
                                prop.active.end_offset prop.active.start_offset
                              )
                              "growing"
                              "shrinking"
                            }}
                        ...
                        <div class="progress-bar-wrapper">
                          <span class="size-bound">{{formatSize0
                              prop.active.start_offset
                            }}</span>
                          <div class="progress-bar-mini">
                            <div
                              class="progress-fill"
      ...
      {{#if this.hasDescription}}
        <div class="character-description">
          ...
          {{#if @character.description}}
            <div
              class="character-full-description"
                    ...
                    {{else}}
                      {{i18n
                        (concat
                          ...
                          (if (eq activity.action_type "grow") "grew" "shrunk")
                        )
                      }}
                    {{/if}}
                    <LinkTo
